...
alpha = input('alpha:   ');
if isempty(alpha) error('an alpha value is required'); end
...
% Initialize arrays to store weights for each iteration
w1_history = zeros(nTrials, 1); 
w2_history = zeros(nTrials, 1); 
w0_history = zeros(nTrials, 1);
...
for iTrial=1:nTrials    % loop over trials (iterations)
   ...
   % Store current weights in history arrays
   w1_history(iTrial) = w(1);
   w2_history(iTrial) = w(2);
   w0_history(iTrial) = w0;
...
plot(1:nTrials, w1_history, 'b-', 'LineWidth', 2); hold on;
plot(1:nTrials, w2_history, 'r-', 'LineWidth', 2);
plot(1:nTrials, w0_history, 'g-', 'LineWidth', 2);
xlabel('Iteration');
ylabel('Weight value');
title('Weights vs. Iteration number');
legend('w1', 'w2', 'w0');
grid on;
