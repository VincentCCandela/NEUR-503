...
def add_bias(inputs):
    ...
    """
    return np.append(inputs,np.ones((1,inputs.shape[1])),axis=0) # add a row of ones to the bottom of the inputs
...
class MLP(object):
    ...
    def __init__(self,N,B,eta,alpha=1.0,sigma=1.0,algorithm='backprop'):
        ...
        self.m_W_y = np.zeros_like(self.W_y)
        self.v_W_y = np.zeros_like(self.W_y)
        self.iteration = 0
    ...
    def gradient(self,inputs,targets):
        ...
        delta_W_y = add_bias(np.dot(hidden, output_error.T)) + 0.1 * self.W_y.T
        hidden_error = np.dot(self.W_y[:,1:].T, output_error) * hidden * (1 - hidden)
        delta_W_h = add_bias(np.dot(inputs, hidden_error.T)) + 0.1 * self.W_h.T
    ...
    #     B = 0.1 * np.random.rand(self.W_y.shape[0], self.W_y.shape[1])

    #     # calculate the updates - CHANGE THESE LINES
    #     output_error = targets - output
    #     delta_W_y = add_bias(np.dot(hidden, output_error.T)) + 0.5 * self.W_y.T
    #     hidden_error = np.dot(B[:,1:].T, output_error) * hidden * (1 - hidden)
    #     delta_W_h = add_bias(np.dot(inputs, hidden_error.T)) + 0.5 * self.W_h.T
    ...
    def feedback(self, inputs, targets):
        ...
        while True:
            # Backpropagation
            output_error = targets - output
            delta_W_y = add_bias(np.dot(hidden, output_error.T)).T

            B = np.random.rand(self.W_y.shape[0], self.W_y.shape[1])

            hidden_error = np.dot(B[:, 1:].T, output_error) * hidden * (1 - hidden)
            delta_W_h = add_bias(np.dot(inputs, hidden_error.T)).T

            # Adam update for W_h
            self.m_W_h = beta1 * self.m_W_h + (1 - beta1) * delta_W_h
            self.v_W_h = beta2 * self.v_W_h + (1 - beta2) * (delta_W_h ** 2)
            m_hat_W_h = self.m_W_h / (1 - beta1 ** self.iteration)
            v_hat_W_h = self.v_W_h / (1 - beta2 ** self.iteration)
            delta_W_h = alpha * m_hat_W_h / (np.sqrt(v_hat_W_h) + epsilon)

            # Adam update for W_y
            self.m_W_y = beta1 * self.m_W_y + (1 - beta1) * delta_W_y
            self.v_W_y = beta2 * self.v_W_y + (1 - beta2) * (delta_W_y ** 2)
            m_hat_W_y = self.m_W_y / (1 - beta1 ** self.iteration)
            v_hat_W_y = self.v_W_y / (1 - beta2 ** self.iteration)
            delta_W_y = alpha * m_hat_W_y / (np.sqrt(v_hat_W_y) + epsilon)

            # Calculate the loss
            temp_W_h = self.W_h + delta_W_h
            temp_W_y = self.W_y + delta_W_y
            if self.loss(inputs, targets, temp_W_h, temp_W_y) < self.loss(inputs, targets, self.W_h, self.W_y):
                break
            count += 1
            if count > 20:
                delta_W_h = 0
    ...
    def kolepoll(self,inputs,targets):
        ...
        A =  (output - targets)*output*(1-output).T@self.V.T

        lam = 0.3

        delta_W_h = A - lam * self.W_h
        delta_W_y = A - lam * self.W_y
        delta_V = A - lam * self.V
